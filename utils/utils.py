...
A module that contains utils for this project.
"""
from io import open
import json
import logging
...
def save_paddle_model():
    params = np.load('params.npz')
    no_transpose_list = ['bert.embeddings.word_embeddings.weight',
                         'bert.embeddings.position_embeddings.weight',
                         'bert.embeddings.token_type_embeddings.weight',
                         'vil_prediction.main.0.weight_g']

    same_shape_list = ['bert.encoder.layer.0.attention.self.query.weight',
    ...
    for k, v in params.items():
        ...
        if len(v.shape) > 2:
            print(v.shape)
            print('this key has more than 2 shape', k)
            exit()

        sub_model = model

        for att in atts:
            sub_model = sub_model.__getattr__(att)

        if 'weight' in k and k not in no_transpose_list:
            v = v.transpose()

        if k == 'vil_prediction.main.0.weight_g' or k == 'vil_prediction.main.3.weight_g':
            v = v.reshape([1,])

        try:
            sub_model.set_value(paddle.to_tensor(v))
        except:
            print(k)
            print(v)
...
def cached_path(url_or_filename, cache_dir=None):
    ...
    """
    if cache_dir is None:
        cache_dir = PADDLE_PRETRAINED_BERT
